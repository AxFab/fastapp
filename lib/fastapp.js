...
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  FastApp.buildFile = function (path, ctx, callback)
  {
    ...
    fs.readFile(path, function (err, data) {
      ...
      var content = FastApp.splitFile(data.toString());
      FastApp.rebuild (content, ctx, callback);
    });
  };


  FastApp.splitFile = function (data) 
  {
    var content = {
      textArr: [],
      commands: [],
    ...
    for(;;) {
      ...
      if (l === null) { break; }
      if (l.index > 0) {
        content.textArr.push(data.substring(0, l.index));
      }
      ...
      content.textArr.push(cmd);
      if (cmd.type !== 'extends') {
        content.commands.push(cmd);
      } else {
        content.template = cmd;
      }
    }

    // Push the rest of the file
    content.textArr.push(data);
    return content;
  };


  FastApp.rebuild = function (content, ctx, callback) 
  {
    // Regroup page parts
    if (content.commands.length === 0) {
      if (content.template) {
        MarkupCommands.extends(content.template, ctx);
      }
      FastApp.regroup(content.textArr, ctx, callback);
    } else {
      // Launch all commands
      async.each(content.commands, function (cmd, cb) {
